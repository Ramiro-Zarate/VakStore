...
export default function CartDrawer() {
  ...
  const dialogTitleId = 'cart-drawer-title'
      ...
      <div ref={trapRef} style={{ display: 'contents' }}>
        <header className={styles.header}>
          <h2 id={dialogTitleId} className={styles.title}>
            Tu carrito
            {count > 0 && (
              <span className={styles.titleCount} aria-label={`${count} ${count === 1 ? 'producto' : 'productos'}`}>
                {count}
              </span>
            )}
          </h2>
        ...
        <div className={styles.body} role="region" aria-label="Productos en el carrito">
          {items.length === 0 ? (
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>Explorá nuestra colección y agregá tu primera camiseta.</p>
              <button type="button" className={styles.shopButton} onClick={closeDrawer}>
                Ver productos
            ...
            <ul className={styles.list} aria-label={`${items.length} ${items.length === 1 ? 'producto' : 'productos'} en el carrito`}>
              {items.map(item => (
                    ...
                    <div className={styles.itemPriceRow}>
                      ...
                      <div className={styles.itemActions}>
                        <div className={styles.quantityControl}>
                          ...
                          <span className={styles.quantityValue} aria-label={`Cantidad: ${item.quantity}`}>
                            {item.quantity}
                          </span>
                          <button
                            type="button"
