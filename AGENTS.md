...
| `PUBLIC_GA_ID` | cliente | Google Analytics 4 Measurement ID (formato `G-XXXXXXX`). Si está setteado, monta el snippet oficial de Google en el `<head>`. **GA4 carga siempre activo** (no opt-in). El `CookieBanner` muestra un aviso informativo con un único botón "Entendido". | Opcional. Si no está, no se monta el script y el banner sigue apareciendo solo con info. Configurado en Vercel 2026-08-03, simplificado el 2026-08-04. |
...
Activación pre-launch de analytics. Decisión final: **GA4 carga siempre activo, sin opt-in**. El `CookieBanner` queda como aviso informativo (1 botón "Entendido") que cumple con la Ley 25.326 de informar al usuario sobre el uso de cookies de análisis.

#### Implementación (versión final 2026-08-04)

- **Env var:** `PUBLIC_GA_ID` (formato `G-XXXXXXX`) en Vercel (3 envs).
- **Script de gtag en `Layout.astro`:** **snippet oficial de Google**, literal, en el `<head>`:
  ```html
  ...
  ```
  Renderizado con `is:inline` y `define:vars` para inyectar `gaId` desde `import.meta.env.PUBLIC_GA_ID`. Si la env var no está seteada, los 2 scripts no se renderizan (chequea `if (gaId)`).
- **CookieBanner:** 1 solo botón "Entendido" (sin opt-in, sin efecto en tracking). Persiste en localStorage con key `cookie-consent='acknowledged'` + `cookie-consent-version='1.1'` + `cookie-consent-date=<ISO>`. Si la versión cambia, el banner reaparece.
- **CSP:** `vercel.json` mantiene allowlist de `googletagmanager.com` (script-src) y `google-analytics.com` (connect-src) — sin cambios.
- **Política de privacidad:** `privacidad.astro` secciones 4 (Terceros) y 5 (Cookies) actualizadas para mencionar que GA4 está siempre activo y cómo evitarlo (bloqueador).

#### Por qué este approach (lección final, 2026-08-04)

El approach anterior usaba 3 scripts separados con consent mode v1 (consent default denied + consent update si accepted + carga condicional de gtag.js). Después de diagnosticar post-deploy, el `gtag('consent', 'default', { analytics_storage: 'denied' })` del primer script **bloqueaba los hits** aunque el user aceptara cookies. Diagnóstico: hits salían con status 200/204, gtag se cargaba OK, consent estaba `granted`, pero Realtime seguía vacío.

Causa raíz más probable: la implementación custom del consent mode era incompatible con algún check interno de GA4 (Data Stream, filtros, o el `wait_for_update` que no se seteaba). **Decisión:** volver al snippet oficial literal de Google (probado por Google con millones de instalaciones, sin riesgo de incompatibilidad). GA4 no requiere opt-in por la Ley 25.326 argentina (solo informar al usuario). El `CookieBanner` queda como capa informativa.

#### Archivos modificados (2026-08-04)

- `src/layouts/Layout.astro` — bloque GA4 movido al `<head>` + `<script async src="...">` nativo + `<script>` con config. Sin consent mode. Sin `appendChild` runtime. Sin localStorage check.
- `src/components/CookieBanner.tsx` — 1 botón "Entendido" + texto mencionando GA4 + `POLICY_VERSION='1.1'` (forzar re-display para users que tenían la versión vieja).
- `src/components/CookieBanner.module.css` — eliminados estilos de `.btnSecondary`. Mobile `flex-direction: column` (antes `column-reverse`).
- `src/pages/privacidad.astro` — `lastUpdated` actualizado a 2026-08-04 + sección 4 (Terceros) agrega Google LLC (Google Analytics 4) + sección 5 (Cookies) menciona GA4 con link a opt-out de Google.

#### Edge cases

- Si `PUBLIC_GA_ID` no está seteado, los 2 scripts no se renderizan. El CookieBanner sigue apareciendo solo con info.
